...
let rec plugc (c1 : ecxt) (c2 : ecxt) : ecxt =
  match c1 with
    Hole -> c2
  | EPred c -> EPred (plugc c c2)
  | ESucc c -> ESucc (plugc c c2)
  | EPlusL (c, e) -> EPlusL (plugc c c2, e)
  | EPlusR (i, c) -> EPlusR (i, plugc c c2)
  | EMultL (c, e) -> EMultL (plugc c c2, e)
  | EMultR (i, c) -> EMultR (i, plugc c c2)
...
let _ = test (decompose (Succ (Pred (Succ (Int 3)))))
             (Some (ESucc (EPred Hole), Succ (Int 3)))
             "succ-pred"

let _ = test (decompose (Mult (Succ (Pred (Succ (Int 3))), Int 5))) 
             (Some (EMultL (ESucc (EPred Hole), Int 5), Succ (Int 3)))
             "multl-succ-pred"

let _ = test (decompose (Mult (Int 5, (Succ (Pred (Succ (Int 3)))))))
             (Some (EMultR (5, (ESucc (EPred Hole))), Succ (Int 3)))
             "multr-succ-pred"

let _ = test (eval (Mult (Int 5, (Succ (Pred (Succ (Int 3)))))))
             (5*(((3+1)-1)+1))
             "eval"
